...
function App() {
    ...
    const compartilharWhatsApp = () => {
        ...
        const mensagem = carrinho.map(item => {
            return `${item.quantidade}x ${item.nome} - R$ ${(produtos[item.nome] * item.quantidade).toFixed(2)}`;
        }).join("\n");

        const total = `Total: R$ ${calcularTotal().toFixed(2)}`;
        const nome = nomeComprador ? `Comprador: ${nomeComprador}` : "";

        const mensagemCompleta = encodeURIComponent(`${nome}\n${mensagem}\n\n${total}`);

        // Link de compartilhamento no WhatsApp
        const linkWhatsApp = `https://api.whatsapp.com/send?text=${mensagemCompleta}`;

        // Abre o WhatsApp com a interface de compartilhamento
    ...
    return (
        <div className="p-4">
            ...
            <div className="list-container">
                {carrinho.length === 0 ? (
                    <p className="mensagem-vazia">Nenhum produto adicionado</p>
                ) : (
                    <ul className="list-disc ml-6 lista-carrinho">
            ...
            <button
                ...
                className="bg-teal-500 text-white p-2 mt-4"
            >
